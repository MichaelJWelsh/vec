 ...
 *      vec because it is out of memory (except for the 'vec_reserve(...)' and 'vec_shrink(...)'
 *      functions which do nothing in such a scenario), it will raise a segment violation.
 *
 *      Fat pointer diagram:
 *          +--------+-------------------------------------------+
 ...
 *
 * FAQ:
 *      1. Is there a difference between 'vec_new_len(...)/vec_len(...)' and 'vec_new_size(...)/vec_size(...)'?
...
VEC_API     void*   vec_new_len(size_t size, size_t nitems);
VEC_API     void*   vec_new_size(size_t size, size_t nitems);
...
VEC_API inline void* vec_new(size_t size) {
    return vec_new_len(size, 1);
}


VEC_API inline void* vec_new_len(size_t size, size_t nitems) {
...
VEC_API inline void* vec_new_size(size_t size, size_t nitems) {
    return vec_new_len(size, nitems);
}
